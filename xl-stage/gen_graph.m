...
function gen_graph(nodes, N)
% Indar: This is the modification of the original matlab script graph_gen.m
% Here we can ask to generate configurable nodes and N
...
%					nranks(i) =  randi([1 ranks], 1);
		...
		% Indar: to avoid error of invalid stream number, let's create
    % the working directory here:
    system(['rm -rf uom_graphs']);
    system(['mkdir  uom_graphs']);
		% Write gv file 
		fname = sprintf("uom_graphs/dag%04i",g);
